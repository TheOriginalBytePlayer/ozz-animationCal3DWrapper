...
CAL3D_WRAPPER_API enum CalBoolean CalCoreModel_AddMeshName(struct CalCoreModel* self, const char* name, int coreMeshId) {
    if (!self || !name || coreMeshId < 0 || coreMeshId >= (int)self->coreMeshes.size()) {
        return False;
    }
...
CAL3D_WRAPPER_API enum CalBoolean CalCoreModel_SetCoreMaterialId(struct CalCoreModel* self, 
                                                                   int coreMaterialThreadId, 
                                                                   int coreMaterialSetId, 
                                                                   int coreMaterialId) {
    if (!self) return False;
    
    self->materialThreads[coreMaterialThreadId][coreMaterialSetId] = coreMaterialId;
    return True;
}
...
CAL3D_WRAPPER_API void CalCoreModel_ReplaceCoreMesh(struct CalCoreModel* self, int coreMeshId, struct CalCoreMesh* pCoreMesh) {
    if (!self || coreMeshId < 0 || coreMeshId >= (int)self->coreMeshes.size() || !pCoreMesh) {
        return;
    }
