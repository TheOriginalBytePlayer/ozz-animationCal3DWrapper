...
CAL3D_WRAPPER_API void CalError_PrintLastError() {
    if (g_lastErrorCode != ERROR_CODE_OK) {
        fprintf(stderr, "Cal3D Error: %s (code %d) at %s:%d\n",
                g_lastError.c_str(), g_lastErrorCode, 
                g_lastErrorFile.c_str(), g_lastErrorLine);
    }
}
...
CAL3D_WRAPPER_API void CalQuaternion_Op_Multiply(struct CalQuaternion* pResult, 
